...
**LLM cost collapse** — every brain mechanical dispatch (HyDE, atoms_gate, self_rag, reasoning_loop, proactive, synthesis, 42 files total) migrated from `openclaw_dispatch` to `brain_core/cli_llm.py`. Fallback chain: `codex exec -m gpt-5.4` → `codex exec -m gpt-5.3-codex-spark` → `claude -p --model sonnet` → `llm_backlog.enqueue` for worst-case catch-up. **$150/day → $0** (all subscription-backed). Avg tokens/call 414K → 5K (80× reduction). OpenClaw gateway reserved for Chris↔Jenna Telegram + skill-heavy agent turns only. **`--skip-git-repo-check` flag required** so codex runs in non-git brain dir.
