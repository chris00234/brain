...
def test_get_summaries_matching_respects_enabled_and_token_overlap(tmp_path, monkeypatch):
    ...
    conn.execute(
        ...
        (
            "h1",
            json.dumps(["brain system", "OpenClaw agents"]),
            "brain/openclaw summary",
            2,
    ...
    assert matches[0]["summary"] == "brain/openclaw summary"
...
def test_clean_entity_name_rejects_operational_noise(monkeypatch):
    ...
    assert community_summaries._clean_entity_name("OpenClaw agents") == "OpenClaw agents"
...
def test_rank_entities_caps_and_prefers_concise_semantic_labels(monkeypatch):
    ...
    assert ranked == ["brain system", "OpenClaw agents", "canonical memory"]
...
def test_persist_summary_stores_clean_capped_entities(tmp_path, monkeypatch):
    ...
    ok = community_summaries._persist_summary(
        "hash1",
        {"brain system", "OpenClaw agents", "/recall/v2", "--flag"},
        "summary",
        2,
    )
    ...
    assert json.loads(rows[0][0]) == ["brain system", "OpenClaw agents"]
