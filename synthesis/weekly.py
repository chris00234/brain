...
dispatches to Sage via brain CLI dispatch (Hermes fallback), writes the result
as a canonical weekly arc.
...
FAILURE_LOG = Path("/Users/chrischo/.hermes/profiles/sage/logs/weekly-synthesis-failures.jsonl")
...
def main() -> None:
    ...
    if not isinstance(parsed.get("did"), list):
        ...
        notify_failure("did field missing or not a list")
        sys.exit(1)
