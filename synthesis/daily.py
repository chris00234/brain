...
def build_prompt(target_date: str, day_results: list[dict]) -> str:
    ...
    Output schema mirrors hermes-agent's context-compressor "handoff"
    framing: explicit Active vs Resolved vs Pending buckets. This prevents
    next-day recall from re-opening already-closed work and surfaces
    what's still blocking Chris without him having to diff yesterday and
    today manually.
    """
    lines = []
    lines.append(f"You are Jenna, Chris's chief of staff. Synthesize Chris's day for {target_date}.")
    lines.append("")
    lines.append("DATA CAPTURED TODAY:")
    lines.append("=" * 60)
    if not day_results:
  ...
  "active_tasks": ["<0-5 things Chris started or kept working on today and has NOT finished. Each a short phrase identifying the work, not a status update.>"],
  "resolved_today": ["<0-5 items that were closed / decided / shipped today. Each concrete enough that a future reader won't re-open it.>"],
  "pending_questions": ["<0-5 questions Chris raised or hit that he did not answer today. Phrase as the actual question.>"],
  ...
  "tomorrow_reflection_question": "<one sentence, derived from today's pattern, that would help Chris reflect tonight. If today was uneventful, return null.>"
    ...
    lines.append("- `resolved_today` and `active_tasks` are mutually exclusive — a thing is EITHER resolved OR still active, never both.")
    lines.append("- `pending_questions` must be questions Chris actually raised (verbatim or paraphrased), not questions you generate on his behalf.")
...
def write_narrative(target_date: str, parsed: dict) -> Path:
    ...
    metadata = {
        ...
        "valid_to": None,
        "sources": [],
        "provenance_summary": f"Auto-generated by Jenna from {target_date} Qdrant fanout.",
    ...
    # Active / Resolved / Pending — hermes-agent handoff framing. Writing
    # these as explicit sections surfaces what's still blocking Chris and
    # prevents tomorrow's recall from re-opening work closed today.
    active = parsed.get("active_tasks") or []
    if active:
        body_lines.append("## Active")
        for a in active:
            body_lines.append(f"- {a}")
        body_lines.append("")
    resolved = parsed.get("resolved_today") or []
    if resolved:
        body_lines.append("## Resolved today")
        for r in resolved:
            body_lines.append(f"- {r}")
        body_lines.append("")
    pending = parsed.get("pending_questions") or []
    if pending:
        body_lines.append("## Pending questions")
        for q in pending:
    ...
    if parsed is None:
        ...
        parsed = {
            "narrative": (
                ...
                "Raw events are still in Qdrant and will be re-synthesized on next run.)_"
            ),
