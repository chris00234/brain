...
from search_unified import search_all  # noqa: E402
from openclaw_dispatch import dispatch as _dispatch  # noqa: E402

try:
    from config import BRAIN_LOGS_DIR, BRAIN_DIR
except ImportError:
...
def _pst_hour(ts_iso: str) -> Optional[int]:
...
def check_schedule_gaps() -> list[ProactiveInsight]:
    ...
    try:
        ...
        for doc, meta in zip(docs, metas):
            ...
            result_list = results.get("results", []) if isinstance(results, dict) else results if isinstance(results, list) else []
        ...
        for domain, stats in accuracy.items():
            ...
            if acc < 0.6:
                ...
                insights.append(ProactiveInsight(
                    ...
                    evidence=[{"domain": domain, "accuracy": acc, "total": total, "overrides": overrides}],
                    generated_at=_now_iso(),
                    expires_at=_expires_iso(48),
                ))
...
def run_proactive_check() -> list[ProactiveInsight]:
    ...
    # Expire old insights
    existing = [
        ins for ins in existing
        if not _is_expired(ins, now)
    ]
    ...
    log.info(
        ...
        len(existing), len(unique_new), len(urgent),
    )
    return existing
