...
def apply_to_result(result: dict, debug: bool = False, now: datetime | None = None) -> dict:
    ...
    Idempotent — re-calling on the same result is a no-op.
    """
    if result.get("_decay_applied"):
        return result
    ...
    mult = time_decay_multiplier(created_at, collection, category=category, now=now)
...
def apply_to_results(results: list[dict], debug: bool = False, now: datetime | None = None) -> list[dict]:
    for r in results:
        apply_to_result(r, debug=debug, now=now)
    return results
