...
def build_enrichment_prompt(contacts: list[dict]) -> str:
    ...
    lines.append('{"keep": [{"index": <int>, "summary": "<1 sentence relationship context>", "signal_score": <1-10>}], "skip_reason": "<why others were skipped>"}')
    lines.append('')
...
def main() -> None:
    ...
    if args.dry_run:
        for c in contacts:
            org = c.get('org', '')
            print(f'  {c["name"]:30s} {c["frequency"]:8s} {c["msg_count"]:3d} msgs  last: {c["last_contact"]}' + (f'  ({org})' if org else ''))
