...
def build_distillation_prompt(agent_name: str, exchanges: list[dict]) -> str:
    ...
    lines.append('{"keep": [{"index": <int>, "summary": "<1-3 sentences>", "signal_type": "decision|preference|diagnosis|correction", "signal_score": <1-10>}], "skip_reason": "<why others were skipped>"}')
    lines.append('')
...
# ── Record Writing ──────────────────────────────────────
def write_record(agent_name: str, session_id: str, session_date: str,
                 item: dict, exchange: dict) -> Path | None:
...
def main() -> None:
    ...
    parser.add_argument('--agents', default=','.join(sorted(ACTIVE_AGENTS)),
