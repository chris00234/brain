...
def build_distillation_prompt(project: str, groups: list[dict]) -> str:
    ...
    lines.append(
        '{"keep": [{"index": <int>, "summary": "<1-3 sentences>", "signal_type": "decision|preference|diagnosis|correction|architecture", "signal_score": <1-10>}], "skip_reason": "<why others were skipped>"}'
    )
    lines.append("")
...
# ── Record Writing ──────────────────────────────────────
def write_record(project: str, session_id: str, session_date: str, item: dict, group: dict) -> Path | None:
...
def main() -> None:
    ...
    for proj_dir in sorted(PROJECTS_DIR.iterdir()):
        ...
        for session_file in session_files:
            ...
            print(
                f"  [{project}] {session_id[:8]}... {len(segments)} segments -> {len(groups)} groups, date={session_date}"
            )
